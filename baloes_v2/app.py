...
Página única: envie a foto, escolha a calibração da cena e os parâmetros,
receba o overlay anotado, o relatório por equipe e as máscaras.
...
def _listar_calibracoes() -> list[str]:
    """Calibrações disponíveis (config/calibracoes/* + calibracao_local.json)."""
    opcoes = sorted(
...
def processar():
    ...
    if not os.path.exists(caminho_calib):
        return render_template_string(
            _PAGINA, overlay=None, avisos=[], calibracoes=calibracoes,
            erro="Calibração não encontrada — gere com tools/calibrar_cores.py.")
